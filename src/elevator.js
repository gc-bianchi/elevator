  ...
  return new Promise((resolve) => {
    rl.question(
      ...
      (answer) => {
        ...
        if (isNaN(currentFloor) || currentFloor < baseFloor || currentFloor > maxNumberOfFloors) {
...
async function main() {
  ...
  const numberOfFloorsVisited = differences.reduce((acc, curr) => acc + curr, 0);

  // multiply number of floors traveled by time it takes to travel to a single floor to calculate total travel time
  const totalTravelTime = numberOfFloorsVisited * singleFloorTravelTimeInSeconds;
